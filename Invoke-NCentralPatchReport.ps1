...
foreach ($file in Get-ChildItem -Path $privatePath -Filter '*.ps1' | Sort-Object Name) {
    Write-Verbose "Loading $($file.Name)"
    . $file.FullName
}

# Dot-source report builder
. (Join-Path $scriptRoot 'Reports\New-PatchManagementReport.ps1')
